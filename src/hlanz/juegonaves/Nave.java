...
public abstract class Nave extends SpriteGameObject {
    ...
    public Nave (int ti, int tde, int tdi, int v, Image image){
        super(image,image.getWidth(null),image.getHeight(null));
    ...
    public void ejecutarFrame() {
        if (consola.getTeclado().teclaPulsada(this.teclaIzquierda)){
            this.moverX(this.velocidad*-1);
        }
        if (consola.getTeclado().teclaPulsada(this.teclaDerecha)){
